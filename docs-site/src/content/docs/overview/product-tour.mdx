...
Every widget runs in a single parallel `Promise.all` inside one `withTenant()` round-trip — see [Dashboard performance + index tuning](/SkillAi/architecture/performance) for the optimisation history.

→ Read more: [System overview](/SkillAi/architecture/system-overview)
...
Each row shows the 20-px agency-logo chip with initials fallback — recruiters instantly see who sourced the candidate. Internal-bench candidates get an **INTERNAL** badge, never any AI scoring boost (see [DEC-010](/SkillAi/decisions/dec-010-internal-bench)).

→ Read more: [Audience-based view sanitisation](/SkillAi/code-patterns/audience-sanitisation)
...
→ Read more: [Audience-based view sanitisation](/SkillAi/code-patterns/audience-sanitisation), [DEC-009 — Soft budget signal](/SkillAi/decisions/dec-009-soft-budget-signal)
...
Margin is computed inline (`role.customerDayRate − candidate.candidateRate`) and shown to recruiters. Over-budget candidates aren't filtered out — Claude is told about the budget as a *soft signal* in the prompt, not a scored dimension (see [DEC-009](/SkillAi/decisions/dec-009-soft-budget-signal)).

→ Read more: [AI scoring pipeline](/SkillAi/architecture/ai-scoring-pipeline), [REST API for scoring](/SkillAi/rest-api/endpoints#scoring)

## 7. Agencies

<Image src={agencies} alt="Agencies management view" />

Recruitment agencies are first-class entities. Each agency has contact details, notes, a logo, and a candidate list with performance metrics. The per-tenant **system "Internal" agency** is auto-provisioned and protected from archival — that's how the internal-bench model surfaces here (see [DEC-010](/SkillAi/decisions/dec-010-internal-bench)).

→ Read more: [Logo upload pattern](/SkillAi/code-patterns/logo-upload)
...
→ Read more: [DEC-006 — Auth.js over Clerk](/SkillAi/decisions/dec-006-authjs-over-clerk), [DEC-011 — GDPR erasure](/SkillAi/decisions/dec-011-gdpr-erasure)
...
These are tracked for the next screenshot pass — see [Live demo](/SkillAi/overview/live-demo) for how to capture them locally.
