---
title: Theme tokens (light & dark)
description: The CSS-variable token system in globals.css, the rules for when to use a token vs a direct Tailwind colour, and the status-pill dark-prefix pattern.
sidebar:
  order: 2
---
...
<Aside type="caution">
Never reintroduce hardcoded `bg-zinc-{700,800,900,950}`, `text-zinc-{100,300,400,500,600}`, or `border-zinc-{600,700,800}` for surfaces. Those classes were the v1 dark-only palette and don't flip under `.light`. The full migration was Epic [#103](https://github.com/olafkfreund/SkillAi/issues/103), shipped across PRs [#161](https://github.com/olafkfreund/SkillAi/pull/161)–[#165](https://github.com/olafkfreund/SkillAi/pull/165). See [DEC-001](/SkillAi/decisions/dec-001-build-vs-buy) for the broader self-hosted, theme-controlled product stance.
</Aside>
