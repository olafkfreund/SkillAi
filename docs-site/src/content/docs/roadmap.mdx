...
<CardGrid>
  <Card title="Commercial & placement">
    Candidate day rates (pay + bill + currency), role customer day rate as budget, real-time margin calculation, soft AI budget signal ([DEC-009](/SkillAi/decisions/dec-009-soft-budget-signal/)), location + language fields, role deadlines with EXPIRED badge ([DEC-008](/SkillAi/decisions/dec-008-manual-archive/)), customer portal links.
  </Card>
  <Card title="Internal bench">
    Per-tenant system "Internal" agency, orthogonal availability enum, derived bench view, INTERNAL badge, bench quick-filter — see [DEC-010](/SkillAi/decisions/dec-010-internal-bench/).
  </Card>
  ...
  <Card title="GDPR (Epic #72)">
    Right-to-erasure UI (Article 17) with explicit transactional deletes + audit redaction — see [DEC-011](/SkillAi/decisions/dec-011-gdpr-erasure/). DSAR export (Article 15) streams a ZIP with 11 JSON files + the original CV + README cover letter.
  </Card>
...
Plus AI cost tracking (closes [#36](https://github.com/olafkfreund/SkillAi/issues/36)), dashboard query batching + 6 perf indexes (PR #173), interview transcript analysis ([DEC-007](/SkillAi/decisions/dec-007-manual-transcript-upload/)), interview slot scheduling with ICS import, in-app help catch-up (PR #172), and a long tail of reliability fixes. For the full PR-by-PR list, see the [canonical roadmap](https://github.com/olafkfreund/SkillAi/blob/main/.agent-os/product/roadmap.md#phase-6-beyond-original-roadmap--shipped).
...
- **File storage migration to Garage** <Badge text="open" variant="note" /> — Phase 5. CVs and logos currently sit on a local Docker named volume. See [DEC-005 — Storage](/SkillAi/decisions/dec-005-storage/) for the upgrade path.
