...
description: Bearer-token auth for the SkillAI REST API and MCP server — token format, argon2id storage, scope inclusion, revocation, and the mint-and-store recipe.
---
...
See [Connecting from Claude Code](/SkillAi/mcp-server/connecting) for the full MCP setup.
...
- **Rate limit hits** are always logged (`api.rate_limit_exceeded`) — see [Rate limiting](/SkillAi/rest-api/rate-limiting).
