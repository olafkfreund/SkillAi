...
description: Postgres dump plus uploads tarball — the operator-level backup procedure, restore steps, and what NOT to confuse with backups.
---
...
- **GDPR per-candidate erasure** ([DEC-011](/SkillAi/decisions/dec-011-gdpr-erasure)) — hard-deletes a candidate's data with audit-log redaction. The companion DSAR export covers the candidate's own data, not the tenant.
...
- [Health & monitoring](/SkillAi/operations/health-monitoring) — verify the DB is healthy before backing up.
- [AWS deployment](/SkillAi/operations/aws-deploy) — RDS snapshots replace the manual `pg_dump` flow in EKS deployments.
- [DEC-011 — GDPR erasure pattern](/SkillAi/decisions/dec-011-gdpr-erasure) — what the per-candidate hard-delete actually does.
