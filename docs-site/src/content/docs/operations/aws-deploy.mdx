...
description: Reference architecture for deploying SkillAI on AWS EKS — RDS for Postgres, EFS for uploads, NLB + cert-manager for TLS.
---
...
- [Backup & recovery](/SkillAi/operations/backup-runbook) — operator-level Postgres dump + uploads tarball procedure.
- [Health & monitoring](/SkillAi/operations/health-monitoring) — `/api/health` endpoint shape and the audit log table.
- [File storage](/SkillAi/architecture/file-storage) — EFS in EKS works identically to the local Docker volume.
