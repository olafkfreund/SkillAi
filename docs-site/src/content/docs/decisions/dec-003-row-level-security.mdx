...
- [DEC-002 — Drizzle ORM over Prisma](/decisions/dec-002-drizzle-over-prisma/) — the ORM choice that made writing RLS-aware queries straightforward via the `withTenant()` helper.
- [DEC-011 — GDPR erasure](/decisions/dec-011-gdpr-erasure/) — every explicit delete runs inside the same `withTenant()` transaction this decision establishes.
