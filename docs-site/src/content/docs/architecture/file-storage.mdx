---
title: File storage
description: Where uploaded CVs and branding logos live, how paths are laid out, and the path-traversal guard.
---
...
The rationale for Garage over MinIO (which moved to AGPL) and over public cloud S3 (data sovereignty) is in [DEC-005](/SkillAi/decisions/dec-005-storage).

## AWS deployment — EFS instead

On AWS EKS, the standard layout is RDS for the database + EFS for uploads. EFS appears as a normal POSIX filesystem mounted at `/app/uploads`, so no application-level changes are needed compared to the Docker volume layout. See [AWS deployment](/SkillAi/operations/aws-deploy) for the full layout.

## Related

- [System overview](/SkillAi/architecture/system-overview) — where storage sits in the request lifecycle.
- [Backup & recovery](/SkillAi/operations/backup-runbook) — how the uploads volume is backed up alongside the database dump.
- [DEC-005 — Local storage + Garage](/SkillAi/decisions/dec-005-storage) — the full decision record.
