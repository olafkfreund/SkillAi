...
description: SkillAI ships a built-in Model Context Protocol server at /api/mcp — 48 tools, 4 resources, 3 prompts, bearer-token auth, write-confirm semantics, full audit trail.
---
...
<CardGrid>
  <Card title="48 tools across 12 modules" icon="puzzle">
    Candidates, roles, agencies, customers, scoring, interviews, approvals, managers, notes, users, settings, exports. Full table at [Tool catalogue](/SkillAi/mcp-server/tools).
  </Card>
  <Card title="4 resources" icon="open-book">
    Candidate, role (with ranked shortlist), interview pack, and "my shortlists" for hiring managers. See [Resources](/SkillAi/mcp-server/resources).
  </Card>
  <Card title="3 prompts" icon="comment-alt">
    Prepare interview brief, weekly shortlist review, candidate introduction email. See [Prompts](/SkillAi/mcp-server/prompts).
  </Card>
  <Card title="Bearer-token auth" icon="seti:lock">
    Same tokens as REST — mint at Settings → API Tokens. See [Authentication](/SkillAi/rest-api/authentication).
  </Card>
</CardGrid>
...
Auth is identical to REST. The bearer token is validated, scoped, and rate-limited by the same code path the REST middleware uses — see [Authentication](/SkillAi/rest-api/authentication) for token format and scope inclusion rules, and [Rate limiting](/SkillAi/rest-api/rate-limiting) for the per-token / per-tenant / per-write sliding windows.
...
- [Connecting from Claude Code](/SkillAi/mcp-server/connecting) — the `claude mcp add` recipe and claude-desktop config.
- [Tool catalogue](/SkillAi/mcp-server/tools) — every tool, scope, and one-line description.
