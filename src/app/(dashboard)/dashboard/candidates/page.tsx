...
export default async function CandidatesPage({ searchParams }: PageProps) {
  ...
  const { q, status, agencyId, availability, page: pageParam } = await searchParams
  ...
  const currentFilters = { q, status, agencyId, availability }
    ...
    <div className="pb-28">
      ...
      {allCandidates.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed
          ...
          {totalCount === 0 && !q && !status && !agencyId && !availability ? (
            <>
