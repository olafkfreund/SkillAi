...
/**
 * Server action: upload (or replace) the logo for a customer.
 *
 * Reads `customerId` and `file` from the FormData.
 ...
 * 3. Fetch the customer to confirm ownership and capture any existing logo_path
 * 4. Persist the new logo file to disk
 * 5. Update customers.logo_path in the DB
...
export async function uploadCustomerLogo(
  ...
  // Fetch the customer to confirm it belongs to this tenant and get the old logo path.
...
/**
 * Server action: remove the logo from a customer.
 *
 * Clears customers.logo_path and best-effort deletes the file from disk.
 */
export async function removeCustomerLogo(
  customerId: string
): Promise<{ success: true } | { success: false; error: string }> {
  const ctx = await getActionContext()
  if (!ctx) return { success: false, error: 'Not authenticated' }
  ...
  // Fetch the customer to confirm ownership and capture the current logo path.
  const [existing] = await withTenant(tenantId, async (tx) =>
    tx
      .select({ id: customers.id, logoPath: customers.logoPath })
      .from(customers)
