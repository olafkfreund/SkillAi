...
function main(): void {
  ...
  // Optional --json <path>: also emits a structured JSON dump for the docs-site
  // catalogue page. Keeps the markdown output unchanged.
  const jsonFlagIdx = process.argv.indexOf('--json')
  if (jsonFlagIdx !== -1 && process.argv[jsonFlagIdx + 1]) {
    const jsonOutPath = path.resolve(process.cwd(), process.argv[jsonFlagIdx + 1])
    const moduleOrder = [
      'candidates',
      'roles',
      'agencies',
    ...
    const byModule: Record<string, ToolEntry[]> = {}
    for (const t of tools) {
      const list = byModule[t.module] ?? []
      list.push(t)
      byModule[t.module] = list
    }
    const orderedModules = moduleOrder
      .filter((m) => byModule[m])
      .map((name) => ({ name, count: byModule[name].length, tools: byModule[name] }))
    ...
    if (!fs.existsSync(path.dirname(jsonOutPath))) {
      fs.mkdirSync(path.dirname(jsonOutPath), { recursive: true })
    }
    fs.writeFileSync(jsonOutPath, JSON.stringify(payload, null, 2), 'utf8')
    console.log(`Wrote JSON catalogue → ${path.relative(process.cwd(), jsonOutPath)}`)
  }
